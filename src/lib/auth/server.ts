...
// Published custom domains (e.g. chat.grok.me) + Vercel previews. Always use a
// *dynamic* baseURL so OAuth redirect_uri matches the host the user is on —
// a fixed BETTER_AUTH_URL of the wrong host breaks sign-in on custom domains.
...
// Missing entries here surface as FORBIDDEN "Invalid origin". Include every
// host family this app may be served from (preview, custom domain, vercel).
...
// Dynamic env read so Nitro/Vite cannot bake DATABASE_URL as empty at build time.
function readDatabaseUrl(): string | undefined {
  ...
  // Split key so static analyzers cannot replace the whole access.
  const dyn = process.env["DATA" + "BASE_URL"];
...
// embedded PGLite (preview only). Never open PGLite on Vercel/Lambda — the
// `pglite.data` WASM asset is missing under `/var/task` and throws ENOENT.
const database = databaseUrl
  ? createAuthPool(databaseUrl)
  : onServerless
    ? createAuthPool(
        // Last-resort placeholder: Better Auth still needs a `database` handle.
        // Prefer a clear connection error over pglite ENOENT if the platform
        // forgot to inject DATABASE_URL. (Publish injects it in normal deploys.)
